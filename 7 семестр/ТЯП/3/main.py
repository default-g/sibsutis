...
def update_automate():
    ...
    try:
        for row in transitions_text.get("1.0", "end").split("\n"):
            row = str(row).strip()
            match = re.fullmatch(r"\s*\(\s*(\w+)\s*,\s*(\w+)\s*,\s*(\w+)\s*\)\s*=\s*\(\s*(\w+)\s*,\s*(\w+)\s*\)\s*", row)
            if not match:
                continue
            ...
            if stackchar not in automate[state][char]:
                automate[state][char][stackchar] = (newstate, newstack)
...
def launch_automate(seq):
    ...
    while stack:
        ...
        output_text.insert(tk.END, f'({state}, {seq[i:] if len(seq[i:]) > 0 else "λ"}, {"".join(reversed(stack))}) ├─ ')
        ...
        i += 1
        new_state, stack_operation = transition3
