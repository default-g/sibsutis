...
void rotate_image(Image* image)
{
    ...
    uint8_t* new_data = malloc(image->head.filesize);

    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int old_offset = i * row_size + j * bytes_per_pixel;

            int new_offset = (width - j - 1) * bytes_per_pixel * height + i * (bytes_per_pixel);

            for (int k = 0; k < bytes_per_pixel; k++) {
                new_data[new_offset + k] = image->rastr[old_offset + k];
            }
        }
    }
    ...
    image->head.width = height;
    image->head.height = width;
}
