...
void write_text_to_image(Image *image, const char *text)
{
    ...
    for (int i = 0; i < strlen(text); i++) {
        for (int j = 0; j < 2; j++) {
            image->rastr[image_rastr_iterator] &= 0xF0;
            image->rastr[image_rastr_iterator] |= ((text[i] >> (j * 4)) & 0x0F);
...
char* extract_text_from_image(Image image)
{
    ...
    memset(text, 0, rastr_size);

    for (int i = 0; i < rastr_size && image_rastr_iterator < rastr_size; i++) {
        for (int j = 0; j < 2; j++) {
            text[i] |= ((image.rastr[image_rastr_iterator] & 0x0F) << (j * 4));
            image_rastr_iterator++;
        }
    }
    return text;
